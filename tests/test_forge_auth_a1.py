...
def test_read_credentials_returns_none_when_file_missing(tmp_path):
    ...
    assert read_api_key_from_credentials_file(missing) is None


def test_read_credentials_returns_key_for_canonical_login_output(tmp_path):
    """Mirrors what `forge login` actually writes to disk."""
    body = (
    ...
    assert read_api_key_from_credentials_file(p) == "fk_live_abc123"


def test_read_credentials_rejects_wrong_prefix(tmp_path):
    p = tmp_path / "creds.toml"
    p.write_text(
        '[forge_auth]\napi_key = "sk-not-forge"\n',
        encoding="utf-8",
    )
    assert read_api_key_from_credentials_file(p) is None


def test_read_credentials_returns_none_when_section_missing(tmp_path):
    p = tmp_path / "creds.toml"
    p.write_text('[other_thing]\napi_key = "fk_live_x"\n', encoding="utf-8")
    assert read_api_key_from_credentials_file(p) is None


def test_read_credentials_returns_none_for_malformed_toml(tmp_path):
    p = tmp_path / "creds.toml"
    p.write_text('[forge_auth\napi_key =', encoding="utf-8")
    assert read_api_key_from_credentials_file(p) is None


def test_read_credentials_accepts_string_path(tmp_path):
    p = tmp_path / "creds.toml"
    p.write_text(
        '[forge_auth]\napi_key = "fk_live_xyz"\n', encoding="utf-8",
    )
    assert read_api_key_from_credentials_file(str(p)) == "fk_live_xyz"
