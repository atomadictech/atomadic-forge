...
import { fCodeSeverity } from "../types";
import { ActionableCard } from "./ui/ActionableCard";
import type { WireReport } from "../types";
...
export function DebtCounter() {
  const { projectRoot, wireReport, debtConfig, setWireReport, setDebtConfig } =
    useForgeStore();
  ...
  useEffect(() => {
    ...
    iRef.current = setInterval(async () => {
      try {
        const r = await client.wire(projectRoot, true);
        setWireReport(r);
      } catch {
  ...
  const debtColor =
    flash === "down"
      ...
      : flash === "up"
        ? "text-cyber-alert"
        : "text-cyber-chrome";
  const f004 = wireReport.violations.filter((v) =>
    v.f_code?.startsWith("F004"),
  ).length;
  const f003 = wireReport.violations.filter((v) =>
    v.f_code?.startsWith("F003"),
  ).length;
  const other = wireReport.violations.filter(
    (v) => !v.f_code?.startsWith("F004") && !v.f_code?.startsWith("F003"),
  ).length;
    ...
    <div className="p-6 max-w-2xl space-y-5">
      <ActionableCard title="Technical Debt" delay={0}>
        ...
        <div className="border-t border-cyber-border pt-4 grid grid-cols-3 gap-2">
          ...
          ).map(([label, cnt, color, border]) => (
            <div
              key={label}
              className={`bg-cyber-dark border ${border} px-3 py-2 text-center`}
            >
              <div className={`font-mono font-bold text-lg ${color}`}>{cnt}</div>
              <div className="font-mono text-[8px] uppercase tracking-widest text-monolith-muted mt-0.5">
                {label}
              </div>
            </div>
          ))}
        ...
        <div className="border-t border-cyber-border pt-4 flex items-center gap-3">
          <label
            htmlFor="hr"
            className="font-mono text-[9px] uppercase tracking-widest text-monolith-muted flex-1"
          >
            Hourly rate (USD)
          </label>
        ...
        <div className="border-t border-cyber-border pt-3 font-mono text-[8px] text-monolith-muted space-y-0.5">
          <div>
            CISQ formula: violations × weight × hourly_rate · polls every 5 s
          </div>
