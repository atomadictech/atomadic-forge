...
export function ComplexityHeatmap() {
  ...
  useEffect(() => {
    ...
    if (!client.complexityScore) {
      setUnavailable(true);
      return;
    }
    const files = [...new Set(scoutReport.symbols.map((s) => s.file))].filter((f) =>
      f.endsWith(".py"),
    );
    ...
    (async () => {
      ...
      try {
        for (const file of files.slice(0, 100)) {
          ...
          const s = await client.complexityScore!(file);
          out.push({ file, score: Math.min(100, Math.max(0, s)) });
        }
        if (!cancelled.current) setComplexities(out);
      } catch {
        if (!cancelled.current) setUnavailable(true);
      }
    })();
    return () => {
        ...
        <ActionableCard title="Complexity Heatmap" delay={0}>
          <div className="border border-dashed border-cyber-border p-5 space-y-3">
            <p className="font-mono text-[10px] uppercase tracking-widest text-cyber-chrome">
              complexipy not available
            </p>
            <p className="font-mono text-[9px] text-monolith-muted">
              Install on the host running forge:
            </p>
