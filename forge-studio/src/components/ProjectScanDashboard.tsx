...
const TC: Record<Tier | "unknown", string> = {
  a0: "#818cf8", a1: "#34d399", a2: "#60a5fa", a3: "#f472b6", a4: "#fb923c", unknown: "#6b7280",
};
const TL: Record<Tier | "unknown", string> = {
  a0: "a0 · Constants", a1: "a1 · Functions", a2: "a2 · Composites",
  a3: "a3 · Features", a4: "a4 · Orchestration", unknown: "Unknown",
};

export function ProjectScanDashboard() {
  const {
    status, projectRoot, scoutReport, wireReport, selectedTier,
    setProjectRoot, setStatus, setError, setScoutReport, setWireReport,
    setTools, setResources, setSelectedTier,
  } = useForgeStore();
  ...
  async function handleScan() {
    ...
    try {
      ...
      await mcp.connect(root);
      setProjectRoot(root); setStatus("connected");
      const [tools, resources, scout, wire] = await Promise.all([
        mcp.toolsList(), mcp.resourcesList(), mcp.callRecon(root), mcp.callWire(root, true),
      ]);
  ...
  const tierFiles = selectedTier && scoutReport
    ...
    <div className="p-6 space-y-5 max-w-4xl">
      ...
      <ActionableCard delay={0}>
        <div
          ...
          onDrop={(e) => { e.preventDefault(); setInputPath(e.dataTransfer.getData("text/plain") || inputPath); }}
        ...
        >
          <div className="flex items-center gap-2 flex-1 bg-cyber-dark border border-cyber-border px-3 py-2 focus-within:border-cyber-cyan transition-colors">
            <FolderSearch size={14} className="text-monolith-muted flex-shrink-0" />
            <input
              type="text"
              value={inputPath}
      ...
      {scoutReport && dist && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {([
              ["Files", scoutReport.file_count, FolderSearch, "text-cyber-cyan"],
              ["Symbols", scoutReport.symbol_count, Zap, "text-indigo-400"],
              ["Violations", wireReport?.violation_count ?? "—", AlertTriangle, wireOk ? "text-cyber-success" : "text-cyber-alert"],
              ["Auto-fix", wireReport?.autofixable_count ?? "—", Wrench, "text-amber-400"],
            ] as [string, number | string, React.ElementType, string][]).map(([label, value, Icon, color], i) => (
              <motion.div
                key={label}
              ...
              >
                ...
                <div className="flex items-center gap-1.5 mt-1">
                  ...
                  <span className="text-[9px] font-mono uppercase tracking-widest text-monolith-muted">{label}</span>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          {wireReport && (
            ...
            >
              <span>{wireOk ? "⬡ Wire: PASS — all imports legal" : `⬡ Wire: FAIL — ${wireReport.violation_count} violation${wireReport.violation_count !== 1 ? "s" : ""}`}</span>
              {!wireOk && (
                <div className="flex gap-3 text-[9px]">
                  {(["F004", "F003", "other"] as const).map((cat) => {
                    const cnt = wireReport.violations.filter((v) =>
                      cat === "other" ? !v.f_code?.startsWith("F004") && !v.f_code?.startsWith("F003") : v.f_code?.startsWith(cat),
                    ).length;
                    if (!cnt) return null;
                    const clr = cat === "F004" ? "text-cyber-alert" : cat === "F003" ? "text-amber-400" : "text-blue-400";
                    return <span key={cat} className={clr}>{cat}× {cnt}</span>;
                  })}
                </div>
              )}
            </motion.div>
              ...
              {tiers.map((tier) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => setSelectedTier(isSelected ? null : (tier as Tier))}
                    role="button"
                  ...
                  >
                    ...
                    <span className="w-8 font-mono text-[10px] text-monolith-muted text-right">{count}</span>
                  </div>
                );
              })}
            </div>
          ...
          {/* Symbol table for selected tier */}
          {selectedTier && (
            <ActionableCard title={`${TL[selectedTier]} — ${tierFiles.length} symbols`} delay={0.15}>
              <div className="max-h-60 overflow-y-auto cyber-scrollbar">
                {tierFiles.slice(0, 200).map((sym, i) => (
                  ...
                  >
                    <span className="text-monolith-muted w-16 flex-shrink-0">{sym.kind}</span>
                    <span className="text-cyber-chrome">{sym.name}</span>
                    <span className="text-monolith-muted ml-auto truncate">{sym.file}:{sym.line}</span>
                  </div>
                ))}
              </div>
            </ActionableCard>
          ...
          {/* Wire violations list */}
          {wireReport && wireReport.violations.length > 0 && (
            <ActionableCard title={`Wire Violations (${wireReport.violations.length})`} delay={0.2}>
              <div className="max-h-60 overflow-y-auto cyber-scrollbar">
                {wireReport.violations.map((v, i) => (
                  ...
                  >
                    <span
                      ...
                      style={{
                        color: v.f_code?.startsWith("F004") ? "#f43f5e"
                          : v.f_code?.startsWith("F003") ? "#fbbf24"
                          : "#60a5fa",
                      }}
                    >
                      {v.f_code}
                    ...
                    <span className="text-monolith-muted ml-auto truncate">{v.file}</span>
                  </div>
                ))}
              </div>
            </ActionableCard>
