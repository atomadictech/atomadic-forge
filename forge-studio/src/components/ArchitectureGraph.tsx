...
const EDGES: [Tier, Tier][] = [
  ["a1","a0"],["a2","a0"],["a2","a1"],["a3","a0"],["a3","a1"],
  ["a3","a2"],["a4","a0"],["a4","a1"],["a4","a2"],["a4","a3"],
];
...
export function ArchitectureGraph() {
  ...
  useEffect(() => {
    ...
    const counts = scoutReport
      ? Object.fromEntries(tiers.map((t) => [t, scoutReport.symbols.filter((s) => s.tier === t).length]))
      : Object.fromEntries(tiers.map((t) => [t, 0]));
    const nodes = tiers.map((tier, i) => ({
      data: { id: tier, label: TL[tier], count: counts[tier] ?? 0, color: TC[tier] },
      ...
      style: ([
        {
          selector: "node",
          style: {
            shape: "round-rectangle", width: 120, height: 56,
            "background-color": "data(color)", "background-opacity": 0.12,
            "border-color": "data(color)", "border-width": 1.5,
            label: "data(label)", "text-valign": "center", "text-halign": "center",
            color: "#94a3b8", "font-size": 10, "font-family": "monospace",
            "white-space": "pre", "text-wrap": "wrap",
          } as Record<string, unknown>,
        },
        {
          selector: "node:selected",
          style: { "background-opacity": 0.35, "border-width": 2.5, color: "#e2e8f0" },
        },
        {
          selector: "edge",
          style: {
    ...
    <div className="p-6 max-w-4xl">
      <ActionableCard title="Architecture Graph" delay={0}>
        <p className="font-mono text-[9px] uppercase tracking-widest text-monolith-muted mb-4">
          5-tier monadic law — edges show allowed upward-only imports · click a node to filter symbols
        </p>
        {scoutReport ? (
          <div
            ref={cyRef}
